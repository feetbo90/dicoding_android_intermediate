...
import com.my.dicoding_android_intermediate.utils.PagedDataSourceTest
...
class StoryRepositoryTest {
    ...
    @Before
    fun setup() {
        storyRepository = StoryRepository(storyDb, apiService)
    }

    @Test
    fun `get stories with pager - successfully`() = runTest {
        val dummyStories = DataTest.generateDummyListStory()
        val data = PagedDataSourceTest.snapshot(dummyStories)
    ...
    fun `get stories with location - successfully`() = runTest {
        ...
        Mockito.`when`(storyRepositoryMock.getAllStoriesWithMapLocation(dummyToken)).thenReturn(expectedResult)

        storyRepositoryMock.getAllStoriesWithMapLocation(dummyToken).collect { result ->
    ...
    @Test
    fun `get stories with map location - throw exception`() = runTest {
        val expectedResponse = flowOf<Result<StoryResponse>>(Result.failure(Exception("failed")))

        Mockito.`when`(storyRepositoryMock.getAllStoriesWithMapLocation(dummyToken)).thenReturn(
            expectedResponse
        )
    ...
    fun `upload image file - successfully`() = runTest {
        val expectedResponse = DataTest.generateDummyFileUploadResponse()
        ...
        Mockito.verifyNoInteractions(storyDb)
    }

    @Test
    fun `upload image - throw exception`() = runTest {
